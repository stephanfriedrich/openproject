...
module Storages
  RSpec.describe OneDriveManagedFolderSyncService, :webmock do
  ...
  shared_let(:single_project_user_remote_identity) do
    ...
    def original_folders
      root_folder_contents.bind { |files_collection| return files_collection.files.select(&:folder?).map(&:id) }
    end
    ...
    def create_folder_for(project_storage, folder_override = nil)
      ...
      Adapters::Input::CreateFolder.build(parent_location: "/", folder_name:).bind do |input_data|
    ...
    def auth_strategy
      Adapters::Registry.resolve("one_drive.authentication.userless").call
    end
  end
end

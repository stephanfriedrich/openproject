      ...
      module Nextcloud
        module Queries
          RSpec.describe CapabilitiesQuery, :webmock do
            ...
            let(:auth_strategy) { Input::Strategy.build(key: :noop) }

            it "is registered as queries.capabilities" do
              expect(Registry.resolve("nextcloud.queries.capabilities")).to eq(described_class)
            end
            ...
            shared_examples_for "a successful Nextcloud capabilities response" do
              it "returns a capabilities object" do
                ...
                expect(response).to be_a(ProviderResults::Capabilities)

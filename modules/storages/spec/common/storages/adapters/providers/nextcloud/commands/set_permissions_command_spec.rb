      ...
      module Nextcloud
        module Commands
          RSpec.describe SetPermissionsCommand, :webmock do
            ...
            let(:test_folder) do
              Input::CreateFolder.build(folder_name: "Permission Test Folder", parent_location: "/VCR").bind do |input_data|
                Registry.resolve("nextcloud.commands.create_folder").call(storage:, auth_strategy:, input_data:).value!
              end
            end
            ...
            context "if a user does not exist",
              ...
              it_behaves_like "adapter set_permissions_command: unknown remote identity"
            end
            ...
            def clean_up(_)
              Input::DeleteFolder.build(location: test_folder.location).bind do |input_data|
